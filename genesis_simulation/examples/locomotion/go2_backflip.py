...
def main():
    ...
    # Use CPU backend for macOS compatibility
    ...
    policy = torch.jit.load(f"./backflip/{args.exp_name}.pt")#.to("mps")
    ...
    gs.tools.run_in_another_thread(fn=run_simulation, args=(env, policy))

    # Start viewer in main thread
    if env.scene.viewer is not None:
